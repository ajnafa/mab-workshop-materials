...
from polars import DataFrame, col, Int64, Float64, Series, when, lit
...
model = CmdStanModel(stan_file='models/betabinomial_thompson_sampling.stan')
...
for t in range(1, T):
    ...
    data_t = (
        data_t
        .group_by(['period', 'treat'], maintain_order=True)
        .agg(
            [
    ...
    if t > 2: 
        ...
        current_data = current_data.insert_column(5, Series('prob_best', prob_best))
        current_data = current_data.insert_column(6, Series('successes_ppd', successes))
        current_data = current_data.insert_column(7, Series('conversion_prob', conversion_prob))
        current_data = current_data.insert_column(8, Series('prior_alpha', stan_data['prior_alpha']))
        current_data = current_data.insert_column(9, Series('prior_beta', stan_data['prior_beta']))
...
gg.ggsave(
    prob_plot, 
    filename=".output/allocations_batch.png",
    height=7,
    width=9
)
...
gg.ggsave(
    prob_est, 
    filename=".output/estimates_batch.png",
    height=7,
    width=9
)